    ...
    for reader_row in reader:
        for column in reader_row:
            ...
            if column[0] == "(":
                ...
                # opening parentheses.
                # And remove leading and trailing single-quote
                if len(latest_row) == 0:
                    column = re.sub(r'^\'|\'$', '', column[1:])
...
def convert(inputFilePath, outputFilePath):
    if os.path.exists(outputFilePath):
        outputFileH = open(outputFilePath, "a")
    else:
        outputFileH = open(outputFilePath, "w")
    try:
        for line in fileinput.input(inputFilePath):
            ...
            if is_insert(line):
                ...
                if values_sanity_check(values):
                    parse_values(values, outputFileH)
